	...
	while (true)
	{
		...
		switch (iLocal_36)
		{
			case 0:
				if (func_22())
				{
					if (func_5(&uLocal_52, "TOWAUD", "TONYA_HANG", 8, 0, 0, 0))
					{
						iLocal_36 = 1;
					}
				}
...
int func_1()//Position - 0x1F1
{
	...
	if ((SYSTEM::VDIST2(Var0, Var1) > 10000f || ENTITY::IS_ENTITY_DEAD(uLocal_37)) || func_4())
	{
	...
	switch (iLocal_39)
	{
		case 0:
			if (ENTITY::DOES_ENTITY_EXIST(uLocal_37) && !ENTITY::IS_ENTITY_DEAD(uLocal_37))
			{
				if (ENTITY::IS_ENTITY_AT_COORD(uLocal_37, Local_48, 1f, 1f, 1f, 0, 1, 0))
				{
					...
					TASK::TASK_PERFORM_SEQUENCE(uLocal_37, uVar2);
					TASK::CLEAR_SEQUENCE_TASK(&uVar2);
					iLocal_39 = 1;
				}
			}
			...
			if (ENTITY::DOES_ENTITY_EXIST(uLocal_37) && !ENTITY::IS_ENTITY_DEAD(uLocal_37))
			{
				if (TASK::GET_SEQUENCE_PROGRESS(uLocal_37) == 1)
				{
					if (!ENTITY::DOES_ENTITY_EXIST(uLocal_38))
					{
						...
						{
							fVar3 = ENTITY::GET_ENTITY_ANIM_CURRENT_TIME(uLocal_37, "amb@world_human_stand_mobile@female@standing@call@enter", "enter");
							if (fVar3 >= 0.157f)
							{
				...
				else if (TASK::GET_SEQUENCE_PROGRESS(uLocal_37) == 2)
				{
					...
					if (iVar4 == 0)
					{
						if (func_5(&uLocal_52, "TOWAUD", "TONYA_CALL1", 8, 0, 0, 0))
						{
							iLocal_39 = 2;
						}
					}
					else if (func_5(&uLocal_52, "TOWAUD", "TONYA_CALL2", 8, 0, 0, 0))
					{
						iLocal_39 = 2;
					}
				}
		...
		case 3:
			...
			if (SYSTEM::VDIST2(Var0, Var1) < 25f)
			{
				...
				{
					if (func_5(&uLocal_52, "TOWAUD", "TONYA_MESS", 8, 0, 0, 0))
					{
						SYSTEM::SETTIMERA(0);
						iLocal_39 = 4;
					}
